...
if(isset($_POST['Submit']))
{
	...
	$Auth_SQL = mysqli_query($conn,"SELECT * from admin_user WHERE username = '$username' AND password = '$password'");
	if(mysqli_num_rows($Auth_SQL) > 0)
	{
		while($User_Array = mysqli_fetch_array($Auth_SQL,MYSQLI_NUM))
		{
			$Auth = array ();
			$Auth = $User_Array;
			$Auth['auth'] = 'true';
