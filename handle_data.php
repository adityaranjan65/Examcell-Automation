...
<div class="container-fluid" style="margin-top:30px; margin-bottom:50px;">
  ...
  $SQLDistinctBatch = mysqli_query($conn,"SELECT DISTINCT yoa,sem,department,batch FROM student_data group by yoa,sem,department,batch ORDER BY sem ASC,batch ASC");
  while ($row = mysqli_fetch_assoc($SQLDistinctBatch))
  {
	  //print_r($row);
